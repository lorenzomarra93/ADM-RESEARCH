...
def run_streamlit_ui():
    """Avvia interfaccia Streamlit per visualizzare timeline + confronto front/back + spettrogrammi.
    Uso:
        streamlit run "analyze_adm_objects (streamlite).py" -- --ui
    Carica un CSV timeline (objects_timeline.csv) e opzionalmente un WAV multicanale.
    """
    try:
        import streamlit as st
    except Exception as e:  # pragma: no cover
        print("Streamlit non disponibile. Installa con 'pip install streamlit'.")
        return
    try:
        import librosa, librosa.display  # noqa
    except Exception:
        st.warning("librosa non disponibile: spettrogramma disabilitato.")
    import matplotlib.pyplot as plt  # locale per UI

    st.set_page_config(page_title="TSF Timeline Viewer", layout="wide")
    st.title("TSF Timeline / Front–Back / Spectrogram Viewer")
    st.markdown("Carica la timeline generata da analyze_adm_objects e opzionalmente il file WAV per lo spettrogramma del canale associato all'oggetto.")

    with st.sidebar:
        st.header("Input")
        csv_file = st.file_uploader("Timeline CSV", type=["csv"])  # objects_timeline.csv
        audio_file = st.file_uploader("Audio multicanale (opzionale)", type=["wav"])  # originale
        n_fft = st.number_input("n_fft", 512, 8192, 2048, step=512)
        hop_length = st.number_input("hop length", 64, 4096, 512, step=64)
        sr_resample = st.number_input("Resample SR", 8000, 96000, 0, step=1000, help="0 = no resample")
        show_mfcc = st.checkbox("Mostra MFCC heatmap (se presenti)", value=True)
        show_corr = st.checkbox("Mostra correlazioni centroid/spread/speed", value=True)
    ...
    if 'time_s' not in df.columns or 'y_fb' not in df.columns:
        st.error("CSV non valido: mancano colonne base (time_s, y_fb).")
        return

    # Preparazione timeline aggregata (conteggio oggetti attivi per frame)
    # Uso grouping su time_s (già normalizzato)
    counts = df.groupby('time_s')['object_id'].nunique().reset_index(name='active_objects')
    # Front / rear counts per frame
    front_mask = df['y_fb'] > AXIS_CENTER_EPS
    rear_mask = df['y_fb'] < -AXIS_CENTER_EPS
    front_counts = df[front_mask].groupby('time_s')['object_id'].nunique().reindex(counts['time_s'], fill_value=0)
    rear_counts = df[rear_mask].groupby('time_s')['object_id'].nunique().reindex(counts['time_s'], fill_value=0)

    st.subheader("Timeline: Active Objects & Front/Rear")
    import altair as alt
    base = pd.DataFrame({
        'time_s': counts['time_s'],
        'Active Objects': counts['active_objects'],
        'Front': front_counts.values,
        'Rear': rear_counts.values,
    })
    chart = alt.Chart(base.melt('time_s')).mark_line().encode(
        x=alt.X('time_s:Q', title='Time (s)'),
        y=alt.Y('value:Q'), color='variable:N'
    ).properties(height=250)
    st.altair_chart(chart, use_container_width=True)

    # Global front/back proportions
    total_frames = len(df['time_s'].unique())
    front_time = (front_counts > 0).sum() / total_frames if total_frames else 0
    rear_time = (rear_counts > 0).sum() / total_frames if total_frames else 0
    center_time = 1.0 - (front_time + rear_time)
    st.markdown(f"**Occupazione temporale (frame con almeno un oggetto):** Front={front_time:.2%} • Rear={rear_time:.2%} • Center={center_time:.2%}")

    st.subheader("Selezione oggetto")
    obj_ids = list(df['object_id'].unique())
    sel_obj = st.selectbox("Object ID", obj_ids)
    dfo = df[df['object_id'] == sel_obj].copy()
    if dfo.empty:
        st.warning("Oggetto senza dati.")
        return

    st.markdown("### Motion & Spatial Series (oggetto)")
    # Plot speed & y_fb & z_height
    fig, ax = plt.subplots(3, 1, figsize=(8, 6), sharex=True)
    ax[0].plot(dfo['time_s'], dfo.get('speed', 0), color='#1f77b4'); ax[0].set_ylabel('Speed')
    ax[1].plot(dfo['time_s'], dfo['y_fb'], color='#d62728'); ax[1].axhline(AXIS_CENTER_EPS, ls='--', c='gray', lw=0.7); ax[1].axhline(-AXIS_CENTER_EPS, ls='--', c='gray', lw=0.7); ax[1].set_ylabel('Y FB')
    ax[2].plot(dfo['time_s'], dfo['z_height'], color='#2ca02c'); ax[2].set_ylabel('Z Height'); ax[2].set_xlabel('Time (s)')
    fig.tight_layout()
    st.pyplot(fig)

    # T-test front vs rear per speed (solo frame dove l'oggetto è presente)
    front_speeds = dfo.loc[dfo['y_fb'] > AXIS_CENTER_EPS, 'speed'].dropna()
    rear_speeds = dfo.loc[dfo['y_fb'] < -AXIS_CENTER_EPS, 'speed'].dropna()
    if len(front_speeds) > 3 and len(rear_speeds) > 3:
        from scipy.stats import ttest_ind
        tval, pval = ttest_ind(front_speeds, rear_speeds, equal_var=False)
        st.markdown(f"**Speed front vs rear (t-test):** t={tval:.2f}, p={pval:.3g}, n_front={len(front_speeds)}, n_rear={len(rear_speeds)}")
    else:
        st.info("Dati insufficienti per t-test (speed front/rear).")

    # MFCC heatmap
    mfcc_cols = [c for c in dfo.columns if re.match(r'^mfcc_\d{2}$', c)]
    if show_mfcc and mfcc_cols:
        mfcc_mat = dfo[mfcc_cols].to_numpy().T
        fig2, ax2 = plt.subplots(figsize=(8, 3))
        im = ax2.imshow(mfcc_mat, aspect='auto', origin='lower', extent=[dfo['time_s'].min(), dfo['time_s'].max(), 1, mfcc_mat.shape[0]])
        ax2.set_ylabel('MFCC'); ax2.set_xlabel('Time (s)'); ax2.set_title('MFCC Heatmap')
        fig2.colorbar(im, ax=ax2)
        st.pyplot(fig2)
    elif show_mfcc:
        st.info("MFCC non presenti nella timeline.")

    # Correlazioni semplici
    if show_corr:
        corr_pairs = [
            ('spec_centroid', 'z_height'),
            ('spec_bandwidth', 'spread'),
            ('rms', 'speed'),
        ]
        corr_text = []
        for a, b in corr_pairs:
            if a in dfo.columns and b in dfo.columns and dfo[a].notna().any() and dfo[b].notna().any():
                r = np.corrcoef(dfo[a].fillna(0), dfo[b].fillna(0))[0, 1]
                corr_text.append(f"{a}↔{b}: r={r:.2f}")
        if corr_text:
            st.markdown("**Correlazioni (oggetto):** " + " • ".join(corr_text))

    # Spettrogramma per l'oggetto (se audio fornito)
    if audio_file is not None:
        try:
            ...
            ch = int(dfo['track_index'].dropna().unique()[0]) if dfo['track_index'].notna().any() else 1
            ch = max(1, min(ch, data.shape[1]))
            y = data[:, ch - 1]
            if sr_resample > 0 and sr_resample != sr:
                y = librosa.resample(y, orig_sr=sr, target_sr=sr_resample)
                sr = sr_resample
            S = np.abs(librosa.stft(y, n_fft=n_fft, hop_length=hop_length)) ** 2
            S_db = librosa.power_to_db(S, ref=np.max)
            fig3, ax3 = plt.subplots(figsize=(9, 4))
            librosa.display.specshow(S_db, sr=sr, hop_length=hop_length, x_axis='time', y_axis='hz', cmap='magma')
            ax3.set_title(f'Spectrogram (ch {ch})')
            fig3.colorbar(ax3.images[0], ax=ax3, format='%+2.0f dB')
            st.pyplot(fig3)
        except Exception as e:
            st.warning(f"Errore spettrogramma: {e}")

    st.caption("TSF Streamlit UI – confronto front/back e analisi timbrica/spaziale.")


if __name__ == '__main__':
    # Se avviato con '--ui' o in un contesto Streamlit (streamlit run), prova UI
    if '--ui' in sys.argv or os.environ.get('STREAMLIT_SERVER_RUNNING'):
        run_streamlit_ui()
    else:
        main()
